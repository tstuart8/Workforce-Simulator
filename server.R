...
function(input, output, session) {
  ...
  observe({
    #################################################################################
    ####################### Calculate FS Point Distribution #########################
    ################################################################################
    ...
    output$fsdist <- renderPlot({
      ggplot(data = dens, aes(x=xfs, y=dxfs)) + 
        geom_area(aes(y = dyfs), fill='Blue') +
    ...
    #################################################################################
    ####################### Calculate DTH Point Distribution #########################
    ################################################################################
    ...
    output$FSHC = renderText({
      paste('Avg. FS Headcount: ', round(mean(district$FS), 0))
    })
    
    output$DTHHC = renderText({
      paste('Avg. DTH Headcount: ', round(mean(district$DTH), 0))
    })
    ...
    output$pointsperweek <- renderText({
      paste('Points Per Week')
    })
    
    output$average_fscompleted <- renderText({
      paste('FS: ', round(mean(final$fstotal), 1))
    })
    
    output$average_dthcompleted <- renderText({
      paste('DTH: ', round(mean(final$dthtotal), 1))
    })
    
    output$average_ticketsperweek <- renderText({
      paste('Total: ', round(mean(final$Points), 1))
    })
    ...
    output$average_fsutil <- renderText({
      paste('FS : ', round(mean(final$fsutil)*100, 0), '%', sep='')
    })
    
    output$average_dthutil <- renderText({
      paste('DTH : ', round(mean(final$dthutil)*100, 0), '%', sep='')
    })
    
    output$servicelevel <- renderText({
      paste('Service Level')
    })
    
    output$average_servicelevel <- renderText({
      paste(round(mean(final$Service), 1), ' Day(s)')
    })
